...
export default class MyListsShow extends React.Component {
  ...
  dateCutoffInput() {
    if (this.props.is_mobile) {
      ...
      return (
        <DatePicker
          selected={this.state.cutoff_date_moment}
          dateFormat="MMMM Do, YYYY"
          className="form-control"
          placeholderText={!this.state.cutoff_date_moment ? "select a date" : null}
          onChange={this.cutoffDateChangeHandlerWithMoment.bind(this)}/>
      )
    }
  }
  
  cutoffDateChangeHandlerWithMoment(cutoff_date_moment) {
    let cutoff_date = cutoff_date_moment ? cutoff_date_moment.format('YYYY-MM-DD') : null
  ...
  cutoffDateSave(cutoff_date, cutoff_date_moment) {
    this.setState({cutoff_date, cutoff_date_moment, cutoff_date_error: null}, function() {
      MyListsApi.updateCutoffDate(this.state.listId, this.state.cutoff_date).then(res => {
        let cutoff_date = res.cutoff_date;
        let cutoff_date_moment = cutoff_date ? moment(cutoff_date) : null;
        this.setState({cutoff_date, cutoff_date_moment, cutoff_date_error: null})
      }, errRes => {
        let cutoff_date_was = errRes && errRes.responseJSON ? errRes.responseJSON.cutoff_date : null
        let cutoff_date_moment_was = cutoff_date_was ? moment(cutoff_date_was) : null;
        let cutoff_date_error = (errRes && errRes.responseJSON && errRes.responseJSON.message) || "Date failed to save"
        this.setState({cutoff_date: cutoff_date_was, cutoff_date_moment: cutoff_date_moment_was, cutoff_date_error})
      })
    })
  }
  ...
  handleGenreChange(genres) {
    if (genres.length > 6) {
      this.setState({genreSelectWarning: 'You may select a maximum of 6 genres'})
    } else {
  ...
  handlePriceClear(priceType) {
    let prices = pick(this.state, ['solo_price', 'feature_price', 'mention_price'])
  ...
  toggleSwapOnly(priceType, event) {
    const state = this.state;
    state[priceType + "_is_swap_only"] = state[priceType + "_is_swap_only"] ? 0 : 1;
    console.log(state)
    this.setState(state, () => {
  ...
  executeSave() {
    MyListsApi.updateInventoriesGenresPrices(
      ...
      this.state.genres,
      pick(this.state, ['solo_price', 'feature_price', 'mention_price', 'solo_is_swap_only', 'feature_is_swap_only', 'mention_is_swap_only'])
    ).then(res => { console.log('saved') }, res => { console.log('save failed') })
  }
  
  changeAuthorEmail(event) {
    let allowedSaveHash = hex(10);
    this.setState({book_owner_email: event.target ? event.target.value : null, lastChangeAt: Date.now(), allowedSaveHash})
  ...
  render() {
    return (
      <div className="my-lists-show">
        <PageTitle title='My Mailing Lists' subtitles={['Set your inventory']}/>
                ...
                <div className="my-lists-show-set-up">
                  ...
                  <div className="my-lists-show-prices">
                    ...
                    <PriceInput 
                      ...
                      changeHandler={this.handlePriceChange.bind(this, 'feature')}
                    ...
                    <PriceInput 
                      ...
                      changeHandler={this.handlePriceChange.bind(this, 'mention')}
                  ...
                  <div className="my-lists-show-genres">
                    <GenreSelect 
                      ...
                      genresOptions={
                        BookclickerStaticData.allGenres.filter(genre => {
                  ...
                  <div className="my-lists-show-cutoff-date">
                    ...
                    <div className="my-lists-show-cutoff-date-error">{this.state.cutoff_date_error}</div>
                  </div>
                  
                  <div className="my-lists-show-inventory">
                    <div className="my-lists-show-inventory-caption">
                      You can always change specific days in your sales calendar.
                    </div>
                    <InventoryInput
                      isMobile={this.props.is_mobile}
                      inventories={this.state.inventories_scaffolded}
                      soloFeatureChangeHandler={this.handleSoloFeatureChange.bind(this)}
                      mentionsChangeHandler={this.handleMentionsChange.bind(this)}/>
                  </div>
                  ...
                  <div className="my-lists-show-submit">
                    <Link 
                      role="button" 
                      to={"/my_lists"}
                      className="bclick-button bclick-solid-robin-egg-blue-button bclick-no-text-transform-button">Save</Link>
                  </div>
                </div>
              </form>
            </div>
