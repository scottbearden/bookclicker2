...
export default class PenNameForm extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      penName: {
        ...
        author_profile_url: this.props.penName ? this.props.penName.author_profile_url : null,
        author_name: this.props.penName ? this.props.penName.author_name : null,
        author_image: this.props.penName ? this.props.penName.author_image : null,
        verified: this.props.penName ? this.props.penName.verified : 0,
        promo_service_only: this.props.penName ? this.props.penName.promo_service_only : 0,
        pen_name_type: this.initialPenNameType(),
        group_status: this.props.penName ? this.props.penName.group_status : null
      }
    }
  }
  ...
  wantsGroupAccess() {
    if (this.state.penName.promo_service_only) {
      ...
      return this.state.penName.group_status == "open" || this.state.penName.group_status == "closed"
    }
  }
  ...
  verifiedOrNotGlyphicon() {
    if (this.state.pingingAmazon) {
      return <span className={"glyphicon glyphicon verified-or-not-glyphicon glyphicon-barcode"}></span>
    } else if (this.state.penName.verified) {
      return <span className={"glyphicon  verified-or-not-glyphicon glyphicon-ok-circle"}></span>
    } else if (this.state.penName.author_profile_url) {
      return <span className={"glyphicon  verified-or-not-glyphicon glyphicon-remove-circle"}></span>
    }
  }
    ...
    this.setState({penName}, function() {
      ...
      //if (match) {
        that.setState({pingingAmazon: true}, function() {
          AmazonProductsApi.getProfileAttrs(url).then(res => {
      ...
      <div className="pen-name-pill-item blank-third">
        <div className="pen-name-pill-item-content">
          <button
            ...
            className={"bclick-button " + (this.props.penName['can_delete?'] ? "bclick-solid-light-red-button" : "bclick-solid-light-grey-button")}
            value={this.state.deleting ? "Deleting..." : "Delete"}>Delete</button>
        </div>
      </div>
    )
  }
  ...
  delete(event) {
    ...
    PenNamesApi.delete(this.state.penName.id).then(res => {
      this.setState({deleting: false})
      document.location = "/pen_names";
    }, errRes => {
      let message = errRes && errRes.responseJSON ? errRes.responseJSON.message : 'Could not delete pen name'
  ...
  save(event) {
    ...
    if (this.wantsVerified() && this.state.pingingAmazon) {
      this.setState({validationError: 'We are still validating your amazon profile url'})
      return null;
    } else if (this.wantsVerified() && !this.state.penName.verified) {
      this.setState({validationError: 'Please enter a verifiable amazon profile url'})
      return null;
    }
    
    let apiCall;
    if (this.state.penName.id) {
      apiCall = PenNamesApi.update.bind(PenNamesApi, this.state.penName.id, this.state.penName);
    } else {
    ...
    this.setState({saving: true, validationError: null}, function() {
      apiCall().then(res => {
        ...
        let message = errRes && errRes.responseJSON ? errRes.responseJSON.message : 'Pen Name failed to save'
      ...
      <div className='clearfix'>
        ...
        <div className="pen-name-pill-item center-input">
          <div className={"pen-name-pill-item-content has-checkbox" + (iOS() ? " ios" : "")}>
            <input type="checkbox" checked={this.wantsGroupAccess()} onChange={this.toggleGroupAccess.bind(this)} />
          </div>
        </div>
        {this.pillThirdSpace()}
      </div>
  ...
  groupAccessSelect() {
    if (this.state.penName.promo_service_only || !this.wantsGroupAccess()) return null;
    return (
      <div className='clearfix'>
        ...
        <div className="pen-name-pill-item center-input">
          <div className="pen-name-pill-item-content">
            <select className="form-control" value={this.state.penName.group_status || ''} onChange={this.changeGroupStatus.bind(this)}>
  ...
  nameOrUrlPromptText() {
    if (this.wantsVerified()) {
      return { header: "Amazon Profile Url", placeholder: "www.amazon.com/William-Shakespeare/e/B000APWKO4"};
    } else if (this.state.penName.promo_service_only) {
  ...
  render() {
    return (
        <div className="pen-name-pill-content">
          ...
          <div className="pen-name-pill-item center-input">
            <div className="pen-name-pill-item-content">
              <select className="form-control" value={this.state.penName.pen_name_type} onChange={this.changePenNameType.bind(this)}>
          ...
          <div className="enter-amazon-profile-text">{this.nameOrUrlPromptText().header}</div>
          {this.pillThirdSpace()}
          <div className="pen-name-pill-item center-input">
            <div className="pen-name-pill-item-content">
          ...
          {this.props.penName && this.props.penName.id ? this.pillDeleteButton() : this.pillThirdSpace()}
          <div className="pen-name-pill-item center-input">
            <div className="pen-name-pill-item-content">
              <input 
                type="submit" 
          ...
          <div className="new-pen-name-validation-error" dangerouslySetInnerHTML={{__html: (this.state.validationError || ' ')}}>
          </div>
