...
export default class RefundRescheduleForm extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = { 
      ...
      rescheduleDateMoment: this.props.booking.date ? moment(this.props.booking.date) : null
    };
  }
      ...
      return (
        <div className={(iOS() ? "ios offer-container" : "offer-container")}>
          ...
          <div className="swap-offer-content fixed-100">
            <label className="refund-amount">Refund ${this.props.booking.refundable_amount}</label>
          ...
          <div className={"respond-button"}>
            <a 
              onClick={
                e => { if (!confirm('Are you sure you want to refund?')) e.preventDefault() }
              }
              href={"/reservations/" + this.props.booking.id + "/refund"}
              className="bclick-button bclick-solid-mailchimp-gray-button respond-button-edit-button">Refund</a>
          </div>
    ...
    if (this.canCancelSwap()) {
      return (
        <div className={(iOS() ? "ios offer-container" : "offer-container")}>
          ...
          <form action={"/reservations/" + this.props.booking.id + "/cancel_swap"} method='POST' onSubmit={e => { if (!confirm('Are you sure you want to cancel this swap?')) e.preventDefault() }} >
            ...
            <div className={"respond-button"}>
              <input
                type="submit" 
                value="Cancel Swap"
                className="bclick-button bclick-solid-mailchimp-gray-button respond-button-edit-button"></input>
            </div>
          </form>
  ...
  datePicker() {
    if (this.props.isMobile) {
      ...
      return (
        <DatePicker
          selected={this.state.rescheduleDateMoment}
          required
          dateFormat="MMMM Do, YYYY"
          className="form-control"
          placeholderText="select a date"
          onChange={this.dateChangeHandlerWithMoment.bind(this)}/>
      )
    }
  }
  
  dateChangeHandlerWithMoment(rescheduleDateMoment) {
    let rescheduleDate = rescheduleDateMoment ? rescheduleDateMoment.format('YYYY-MM-DD') : null
  ...
  rescheduleContainer() {
    if (true) {
      return (
        <div className={(iOS() ? "ios offer-container" : "offer-container") + (!this.canRefund() && !this.canCancelSwap() ? " width-100" : "")}>
          
          <form action={"/reservations/" + this.props.booking.id + "/reschedule"} method='POST' onSubmit={this.validateRescheduleForm.bind(this)} >
  ...
  validateRescheduleForm(event) {
    if (this.invalidRescheduleDate()) {
      ...
      this.setState({respondError: "You must select a date that hasn't passed"})
    } else {
      return null;
    }
  }
    ...
    return (
      <div className="booking-form respond-booking-form">
        <div className="booking-form-title">
          {!this.canRefund() ? "Reschedule" : "Edit"} <a href={"/reservations/" + this.props.booking.id + "/info"} target="_blank">{this.props.booking.inv_type.capitalize()} Offer</a>
